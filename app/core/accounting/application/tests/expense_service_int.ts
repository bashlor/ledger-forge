...
test.group('ExpenseService | confirmExpense', (group) => {
  ...
  test('confirms a draft expense', async ({ assert }) => {
    const created = await service.createExpense(makeInput(), SYSTEM_ACCOUNTING_ACCESS_CONTEXT)
    const confirmed = await service.confirmExpense(created.id, SYSTEM_ACCOUNTING_ACCESS_CONTEXT)
  ...
  test('throws not_found when expense does not exist', async ({ assert }) => {
    const error = await service
      .confirmExpense('nonexistent-id', SYSTEM_ACCOUNTING_ACCESS_CONTEXT)
      .catch((e) => e)
  ...
  test('throws business_logic_error when expense is already confirmed', async ({ assert }) => {
    const created = await service.createExpense(makeInput(), SYSTEM_ACCOUNTING_ACCESS_CONTEXT)
    await service.confirmExpense(created.id, SYSTEM_ACCOUNTING_ACCESS_CONTEXT)

    const error = await service
      .confirmExpense(created.id, SYSTEM_ACCOUNTING_ACCESS_CONTEXT)
      .catch((e) => e)
...
test.group('ExpenseService | deleteExpense', (group) => {
  ...
  test('deletes a draft expense', async ({ assert }) => {
    const created = await service.createExpense(makeInput(), SYSTEM_ACCOUNTING_ACCESS_CONTEXT)
    await service.deleteExpense(created.id, SYSTEM_ACCOUNTING_ACCESS_CONTEXT)

    const list = await service.listExpenses(1, 5, SYSTEM_ACCOUNTING_ACCESS_CONTEXT)
  ...
  test('throws not_found when expense does not exist', async ({ assert }) => {
    const error = await service
      .deleteExpense('nonexistent-id', SYSTEM_ACCOUNTING_ACCESS_CONTEXT)
      .catch((e) => e)
  ...
  test('throws business_logic_error when expense is confirmed', async ({ assert }) => {
    ...
    const error = await service
      .deleteExpense(created.id, SYSTEM_ACCOUNTING_ACCESS_CONTEXT)
      .catch((e) => e)
...
test.group('ExpenseService | listExpenses', (group) => {
  ...
  test('paginates correctly', async ({ assert }) => {
    ...
    const page1 = await service.listExpenses(1, 3, SYSTEM_ACCOUNTING_ACCESS_CONTEXT)
    ...
    const page2 = await service.listExpenses(2, 3, SYSTEM_ACCOUNTING_ACCESS_CONTEXT)
    assert.equal(page2.items.length, 3)
    assert.equal(page2.pagination.page, 2)

    const page3 = await service.listExpenses(3, 3, SYSTEM_ACCOUNTING_ACCESS_CONTEXT)
  ...
  test('summary counts only confirmed expenses for totalAmount', async ({ assert }) => {
    ...
    await service.confirmExpense(toConfirm1.id, SYSTEM_ACCOUNTING_ACCESS_CONTEXT)
    await service.confirmExpense(toConfirm2.id, SYSTEM_ACCOUNTING_ACCESS_CONTEXT)
  ...
  test('clamps page to valid range', async ({ assert }) => {
    await service.createExpense(makeInput(), SYSTEM_ACCOUNTING_ACCESS_CONTEXT)

    const tooHigh = await service.listExpenses(999, 5, SYSTEM_ACCOUNTING_ACCESS_CONTEXT)
    assert.equal(tooHigh.pagination.page, 1)

    const tooLow = await service.listExpenses(-1, 5, SYSTEM_ACCOUNTING_ACCESS_CONTEXT)
  ...
  test('filters by date range', async ({ assert }) => {
    ...
    const result = await service.listExpenses(1, 10, SYSTEM_ACCOUNTING_ACCESS_CONTEXT, {
      endDate: '2026-02-28',
  ...
  test('getSummary respects date filter', async ({ assert }) => {
    ...
    await service.confirmExpense(jan.id, SYSTEM_ACCOUNTING_ACCESS_CONTEXT)

    const all = await service.getSummary(SYSTEM_ACCOUNTING_ACCESS_CONTEXT)
    assert.equal(all.totalCount, 2)
    assert.equal(all.totalAmount, 10)

    const janOnly = await service.getSummary(SYSTEM_ACCOUNTING_ACCESS_CONTEXT, {
      endDate: '2026-01-31',
    ...
    const febOnly = await service.getSummary(SYSTEM_ACCOUNTING_ACCESS_CONTEXT, {
      endDate: '2026-02-28',
      startDate: '2026-02-01',
    })
