...
} from './invoice_mutations.js'

test.group('invoice domain mutations', () => {
  test('builds the draft invoice write model and totals', ({ assert }) => {
    const preparedLines = buildDraftInvoiceLinesMutation([
    ...
    const draftWrite = buildDraftInvoiceMutation({
      customer,
  ...
  test('builds the issue snapshot write model', ({ assert }) => {
    const sendWrite = buildInvoiceIssueMutation({
      customer: {
        address: '10 rue de la Paix, 75002 Paris',
        company: 'Acme',
        email: 'billing@acme.test',
