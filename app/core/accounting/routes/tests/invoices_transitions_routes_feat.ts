...
test.group(
  ...
  (group) => {
    ...
    test('member can read invoices but cannot mark an invoice as paid', async ({
      assert,
      client,
    }) => {
    ...
    test('admin can issue a draft invoice via HTTP', async ({ assert, client }) => {
      await setInvoiceActorRole(db, 'admin')
      const draft = await createDraftViaHttp(db, client)
    ...
    test('inactive membership cannot create, update, or delete draft invoices', async ({
      assert,
      client,
    }) => {
